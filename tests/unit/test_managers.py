...
from controllers.login_manager import AuthenticationAndPermissions, MenuManager
from models.models import Client, Staff
...
FAKE_HASHED_PASSWORD = (
    "$argon2id$v=19$m=65536,t=3,p=4$AcB4D8GYc27tnRPinNM6hw$2PfVv4W" "H1CoD0/TWmyDw2z0iBTj++WAVt/G9IJRnvtg"
)
...
class TestLogin:
    def test_check_password(self, mocker):
        ...
        mock_staff_user = mock_session.query.return_value.filter.return_value.one_or_none.return_value
        mock_staff_user.password = FAKE_HASHED_PASSWORD
        mock_staff_user.department.name = "COMMERCIAL"

        mock_create_token = mocker.patch("controllers.login_manager.AuthenticationAndPermissions.create_token")
...
class TestCrud:
    def test_create_client(self, mocker, get_datas_create_client_fixture, staff_user_and_token_fixture):
        mocker.patch("views.get_datas.GetDatas.get_create_datas", return_value=get_datas_create_client_fixture)
        mocker.patch("controllers.permissions.Permissions.permission_create", return_value=True)
        sut = CrudManager(staff_user_and_token_fixture[0], staff_user_and_token_fixture[1])
        assert sut.create("client") == "creation_ok"

    @classmethod
    def teardown_class(cls):
        client = SESSION.query(Client).filter(Client.fullname == "Cyril Dupont").first()
        SESSION.delete(client)
        clientupdate = SESSION.query(Client).filter(Client.id == 6).first()
        clientupdate.fullname = "Henri Dupont"
        SESSION.commit()

    def test_create_contract(self, mocker, get_datas_create_contract_fixture, staff_user_and_token_fixture):
        mocker.patch("views.get_datas.GetDatas.get_create_datas", return_value=get_datas_create_contract_fixture)
        mocker.patch("controllers.permissions.Permissions.permission_create", return_value=True)
        sut = CrudManager(staff_user_and_token_fixture[0], staff_user_and_token_fixture[1])
        assert sut.create("contract") == "creation_ok"

    """
    def test_create_event(self, mocker, get_datas_create_event_fixture, staff_user_and_token_fixture):
        mocker.patch("views.get_datas.GetDatas.get_create_datas", return_value=get_datas_create_event_fixture)
        mock_fullname = mocker.patch("views.get_datas.GetDatas.get_fullname")
        mock_fullname.return_value = "Martin Durand"
        mock_client = mocker.patch("models.repository.ClientRepository.find_by_fullname")
        client_repository = ClientRepository()
        client = client_repository.find_by_fullname("Martin Durand")
        mock_client.return_value = client
        mocker.patch("models.repository.ClientRepository.find_by_fullname.id", return_value=2)
        mocker.patch("controllers.permissions.Permissions.permission_create", return_value=True)
        mock_permission_is_own_client = mocker.patch("controllers.permissions.Permissions.is_own_client")
        mock_permission_is_own_client.return_value = True
        sut = CrudManager(staff_user_and_token_fixture[0], staff_user_and_token_fixture[1])
        assert sut.create("event") == "creation_ok"
    """

    def test_create_staff(self, mocker, get_datas_create_staff_fixture, staff_user_and_token_fixture):
        mocker.patch("views.get_datas.GetDatas.get_create_datas", return_value=get_datas_create_staff_fixture)
        mocker.patch("controllers.permissions.Permissions.permission_create", return_value=True)
        sut = CrudManager(staff_user_and_token_fixture[0], staff_user_and_token_fixture[1])
        assert sut.create("staff") == "creation_ok"

    def test_read_client(self, mocker, staff_user_and_token_fixture):
    ...
    def test_read_contract(self, mocker, staff_user_and_token_fixture):
        mocker.patch("views.menu.Menu.view_menu_read_only", return_value=3)
        mocker.patch("views.get_datas.GetDatas.get_id", return_value=1)
        sut = CrudManager(staff_user_and_token_fixture[0], staff_user_and_token_fixture[1])
        assert sut.read("contract") is True

    def test_read_event(self, mocker, staff_user_and_token_fixture):
        mocker.patch("views.menu.Menu.view_menu_read_only", return_value=3)
        mocker.patch("views.get_datas.GetDatas.get_id", return_value=1)
        sut = CrudManager(staff_user_and_token_fixture[0], staff_user_and_token_fixture[1])
        assert sut.read("event") is True

    def test_read_staff(self, mocker, staff_user_and_token_fixture):
        mocker.patch("views.menu.Menu.view_menu_read_only", return_value=2)
        mocker.patch("views.get_datas.GetDatas.get_id", return_value=1)
        sut = CrudManager(staff_user_and_token_fixture[0], staff_user_and_token_fixture[1])
        assert sut.read("staff") is True

    def test_update_client(self, mocker, staff_user_and_token_fixture):
        mocker.patch("views.get_datas.GetDatas.get_fullname", return_value="Cyril Dupont")
        mocker.patch("controllers.permissions.Permissions.permission_update", return_value=True)
        mocker.patch("views.menu.Menu.choice_column_to_update", return_value=1)
        mocker.patch("views.get_datas.GetDatas.get_new_value", return_value="Henri Dupond")
        sut = CrudManager(staff_user_and_token_fixture[0], staff_user_and_token_fixture[1])
        assert sut.update("client") == "update_ok"

    def test_update_contract(self, mocker, staff_user_and_token_fixture):
        mocker.patch("views.get_datas.GetDatas.get_id", return_value=1)
        mocker.patch("controllers.permissions.Permissions.permission_update", return_value=True)
...
class TestMenuManager:
    def test_main_menu(self, mocker, staff_user_and_token_fixture):
        mocker.patch("views.menu.Menu.main_menu", return_value=1)
        mock_choice_submenu = mocker.patch("controllers.menu_manager.MenuManager.choice_submenu")
        choice_submenu = menu_manager.MenuManager(staff_user_and_token_fixture[0], staff_user_and_token_fixture[1])
        choice_submenu.choice_main_menu()
        mock_choice_submenu.assert_called_once()
        mock_choice_submenu.assert_called_once_with("client")

    def test_choice_submenu(self, mocker, staff_user_and_token_fixture):
        mocker.patch("views.menu.Menu.submenu", return_value=2)
        mocker.patch("controllers.crud_manager.CrudManager.create", return_value="creation_ok")
        ...
        mock_choice_main_menu = mocker.patch("controllers.login_manager.MenuManager.choice_main_menu")

        choice_crud_create = menu_manager.MenuManager(staff_user_and_token_fixture[0], staff_user_and_token_fixture[1])
